...
把上传的病历 docx 分两阶段处理：先「图转文 + 脱敏」，用户确认后再「结构化提取 + 输出表格」。外部能力由远程服务提供（MinerU API + Ollama），确定性处理由脚本完成。

## 远程服务（2026-09 移机后部署，已固化在 `references/mcp_client/mcp_client.py`）

| 服务 | 地址 | 用途 |
|---|---|---|
| MinerU 官方 API（WSL 部署） | `http://10.20.79.38:8888`（`/file_parse` 上传解析、`/health` 健康检查） | 图转文 OCR，文件以 multipart 上传，**无需任何路径映射** |
| Ollama（同机 GPU） | `http://10.20.79.38:11434`（`/api/chat`） | qwen3:32b 敏感词识别 |

端点可用环境变量 `MINERU_API_URL` / `OLLAMA_URL` 覆盖；连通性自检：`python references/mcp_client/mcp_client.py`。

## 能力总览

| 阶段 | 步骤 | 能力 | 载体 |
|---|---|---|---|
| 一 | 图转文 | docx → Markdown(每图一个一级标题) | `references/mcp_client/mineru_call.py`（本地解包 docx 提取内嵌图片 → 分批上传远程 MinerU `pipeline` 后端 OCR；纯文本 docx 整份上传） |
| 一 | 脱敏 | 识别医院名/人名 + 掩码 | `references/mcp_client/llm_sensitive.py`（直连远程 Ollama qwen3:32b）+ `desensitize/scripts/mask.py` |
...
1. `docx-ocr`：docx → Markdown（用 `references/mcp_client/mineru_call.py`，长超时直连远程 API）。
...
3. **长任务（图转文/批次识别）一律走固定直连库 `references/mcp_client/`**：会话内 MCP 客户端约 2 分钟超时，整份病历 OCR 需 5~8 分钟，故不得用会话内工具做长任务。改用 `references/mcp_client/mineru_call.py` / `llm_sensitive.py`（直连远程 MinerU `10.20.79.38:8888` / Ollama `10.20.79.38:11434`，长超时默认 1800s）。**禁止为单个任务临时手写 HTTP 客户端脚本**——如有新需求，先把能力补进 `references/mcp_client/` 库再复用。
...
- 远程服务不可用时：不阻塞，先跑 `python references/mcp_client/mcp_client.py` 自检连通性（会打印两个服务的健康状态），提示用户确认内网机器 `10.20.79.38` 上的 MinerU API（8888）/ Ollama（11434）是否在跑，或用环境变量 `MINERU_API_URL` / `OLLAMA_URL` 指向新地址。
...
- [mcp_client/（固定直连库：mcp_client.py / mineru_call.py / llm_sensitive.py）](references/mcp_client/)
